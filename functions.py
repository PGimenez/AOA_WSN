...
class Network():
    ...
    def __init__(self, N, dim_x, dim_y):
        self.N = N
        self.dim_x = dim_x
        self.dim_y = dim_y

    def measure_AOA(self, self.mt_position, self.node_positions, self.ind_NLOS, self.std, noise = 1):
        distance_xy = mt_position - node_positions
        distance = np.sqrt(np.sum(distance_xy**2,1))
        AOA = np.arctan2(mt_position[:,0], mt_position[:,1]) + noise * std*np.random.randn(N)

    def gen_graph(self, dim_x, dim_y):
        self.node_positions = np.random.uniform(0, 1, (N,2)) * [self.dim_x, self.dim_y]
